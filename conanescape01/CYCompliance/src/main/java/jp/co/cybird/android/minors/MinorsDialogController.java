...
class MinorsDialogController extends ControllerBase implements DialogFragmentListener {
    ...
    public void show(FragmentManager fragmentManager) {
        ...
        if (!MinorsPref.sharedInstance(mContext).isAgeRegist(mEulaVer)) {
            ...
            int bYear = MinorsPref.sharedInstance(mContext).getBirthYear();
            int bMonth = MinorsPref.sharedInstance(mContext).getBirthMonth();
            if (Util.isMinor(bYear, bMonth, mCurrentCalendar)) {
                enableDismiss = false;
                createMinorsDialogFragment();
                mCurrentDialog.show(mFragmentManager);
            }
    ...
    /**
     * WebViewClientのイベントを通知する //OnWebViewClientEventLinstener
     *
     * @param view
     * @param url
     * @param favicon
    ...
    public void onPageStarted(WebView view, String url, Bitmap favicon) {
        if (mCurrentDialog != null) {
            View button = mCurrentDialog.getButton(CustomDialogFragment.BUTTON_POSITIVE);
            if (button != null) {
